...
class TestCoordinateTransformer:
    ...
    def test_transformer_initialization(self):
        ...
        transformer = CoordinateTransformer(source_crs='EPSG:4326', target_crs='EPSG:3857')
    ...
    def test_inverse_transform(self):
        ...
        x_transformed, y_transformed = transformer.transform_coordinates(original_lons, original_lats)
        lons_back, lats_back = transformer.inverse_transform(x_transformed, y_transformed)
...
class TestTerrainAnalyzer:
    ...
    def test_calculate_slope(self):
        ...
        elevation = np.array([[100, 101, 102], [103, 104, 105], [106, 107, 108]], dtype=np.float32)
    ...
    def test_calculate_aspect(self):
        ...
        elevation = np.array([[100, 100, 100], [101, 101, 101], [102, 102, 102]], dtype=np.float32)
        pixel_size = 30.0
...
class TestVegetationAnalyzer:
    ...
    def test_classify_vegetation_density(self):
        ...
        assert np.all(vegetation_classes >= 0)  # Classes should be non-negative integers
    ...
    def test_detect_vegetation_anomalies(self):
        ...
        ndvi_data = np.array([
            [0.8, 0.8, 0.8],
            [0.8, 0.1, 0.8],  # Anomaly at center
            [0.8, 0.8, 0.8]
        ], dtype=np.float32)
        
        anomalies = analyzer.detect_vegetation_anomalies(ndvi_data, window_size=3, threshold=0.5)
...
class TestSpatialFeatureExtractor:
    ...
    def test_extract_textural_features(self):
        ...
        for feature_name, feature_array in texture_features.items():
            ...
            assert feature_array.shape[0] <= image.shape[0]  # May be smaller due to window
...
class TestArchaeologicalSiteDetector:
    ...
    def test_identify_potential_sites(self):
        ...
        potential_sites = detector.identify_potential_sites(features, confidence_threshold=0.7)
    ...
    def test_cluster_nearby_detections(self):
        ...
        clustered_sites = detector.cluster_nearby_detections(detections, max_distance=10)
        
        assert isinstance(clustered_sites, list)
        assert len(clustered_sites) <= len(detections)  # Should have fewer or equal clusters
