...
class RasterProcessor:
    ...
    def __init__(
        self, nodata_value: Optional[float] = None, resampling_method: str = "bilinear"
    ):
    ...
    def resample_raster(
        self, input_data: np.ndarray, input_transform: Any, target_resolution: float
        ...
        new_transform = input_transform * input_transform.scale(
            input_data.shape[1] / new_width, input_data.shape[0] / new_height
        )
        ...
        zoom_factors = (
            new_height / input_data.shape[0],
            new_width / input_data.shape[1],
        )
    ...
    def align_rasters(self, rasters: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
        ...
        min_bounds = [
            max(b[0] for b in all_bounds),  # left
...
class TerrainAnalyzer:
    ...
    def calculate_curvature(
        self, elevation: np.ndarray, pixel_size: float
    ) -> Dict[str, np.ndarray]:
        ...
        profile_curvature = (
            grad_xx * grad_x**2 + 2 * grad_xy * grad_x * grad_y + grad_yy * grad_y**2
        ) / grad_mag**3

        # Plan curvature (curvature perpendicular to slope direction)
        plan_curvature = (
            grad_xx * grad_y**2 - 2 * grad_xy * grad_x * grad_y + grad_yy * grad_x**2
        ) / grad_mag**2
    ...
    def identify_terrain_features(
        ...
        peaks_mask = (elevation == local_maxima) & (
            elevation > np.percentile(elevation, 95)
        )
        peaks = list(zip(*np.where(peaks_mask)))

        # Local minima (valleys)
        local_minima = minimum_filter(elevation, size=3)
        valleys_mask = (elevation == local_minima) & (
            elevation < np.percentile(elevation, 5)
        )
        ...
        ridge_mask = curvature["profile_curvature"] > np.percentile(
            curvature["profile_curvature"], 90
        )
        ridges = list(zip(*np.where(ridge_mask)))

        return {"peaks": peaks, "valleys": valleys, "ridges": ridges}
...
class VegetationAnalyzer:
    ...
    def classify_vegetation_density(self, ndvi_data: np.ndarray) -> np.ndarray:
        ...
        classes = np.where(
            ndvi_data < self.threshold_values["bare_soil"], 0, classes
        )  # Bare soil
        classes = np.where(
            (ndvi_data >= self.threshold_values["bare_soil"])
            & (ndvi_data < self.threshold_values["sparse"]),
            1,
            classes,
        )  # Sparse
        classes = np.where(
            (ndvi_data >= self.threshold_values["sparse"])
            & (ndvi_data < self.threshold_values["moderate"]),
            2,
            classes,
        )  # Moderate
        classes = np.where(
            (ndvi_data >= self.threshold_values["moderate"])
            & (ndvi_data < self.threshold_values["dense"]),
            3,
            classes,
        )  # Dense
        classes = np.where(
            ndvi_data >= self.threshold_values["dense"], 4, classes
        )  # Very dense

        return classes

    def detect_vegetation_anomalies(
        self, ndvi_data: np.ndarray, window_size: int = 3, threshold: float = 0.5
    ) -> np.ndarray:
    ...
    def calculate_vegetation_indices(
        ...
        ndvi_den = nir + red
        ndvi = np.divide(
            ndvi_num, ndvi_den, out=np.zeros_like(ndvi_num), where=ndvi_den != 0
        )
        ...
        savi_den = nir + red + L
        savi = np.divide(
            savi_num, savi_den, out=np.zeros_like(savi_num), where=savi_den != 0
        )
        ...
        evi_den = nir + 6 * red + 1
        evi = np.divide(
            evi_num, evi_den, out=np.zeros_like(evi_num), where=evi_den != 0
        )
...
class SpatialFeatureExtractor:
    ...
    def extract_textural_features(
        self, image: np.ndarray, window_size: int = 7
    ) -> Dict[str, np.ndarray]:
...
class ArchaeologicalSiteDetector:
    ...
    def identify_potential_sites(
        self, features: np.ndarray, confidence_threshold: float = 0.7
    ) -> List[Dict[str, Any]]:
    ...
    def validate_site_characteristics(
    ...
    def cluster_nearby_detections(
        self, detections: List[Dict[str, Any]], max_distance: float = 10
    ) -> List[ClusterResult]:
    ...
    def generate_site_report(self, site: Dict[str, Any]) -> Dict[str, Any]:
        ...
        feature_desc = (
            ", ".join(terrain_features) if terrain_features else "Unknown features"
        )

        description = (
            f"Potential archaeological site identified with {feature_desc}. "
        ...
        return {
            ...
            "confidence_assessment": {"score": confidence, "level": confidence_level},
