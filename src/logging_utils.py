...
class CustomFormatter(logging.Formatter):
    ...
    def format(self, record):
        if self.use_colors and record.levelname in self.COLORS:
            ...
            record.msg = f"{self.COLORS[record.levelname.strip()]}{record.msg}{self.RESET}"
    ...
    def log_operation(self, operation: str, level: str = "INFO"):
        ...
        def decorator(func):
            ...
            def wrapper(*args, **kwargs):
                ...
                self.logger.info(f"Starting {operation}", extra={'operation': operation})
...
class ErrorHandler:
    ...
    def handle_data_error(self, error: Exception, data_type: str, file_path: str = None):
        ...
        if "FileNotFoundError" in str(type(error)):
            self.logger.logger.info("Recovery suggestion: Check file path and ensure data exists")
            ...
            self.logger.logger.info("Recovery suggestion: Process data in smaller chunks")
    ...
    def handle_api_error(self, error: Exception, api_name: str, retry_count: int = 0):
        ...
        if "rate limit" in str(error).lower():
            self.logger.logger.info("Recovery suggestion: Implement exponential backoff")
    ...
    def handle_processing_error(self, error: Exception, operation: str, data_info: Dict[str, Any] = None):
