...
class DataPaths:
    ...
    def __post_init__(self):
        ...
        for path in [self.raw_data_dir, self.processed_data_dir, 
                    self.models_dir, self.outputs_dir, self.logs_dir]:
...
class GeospatialConfig:
    ...
    amazon_bounds: Dict[str, float] = field(default_factory=lambda: {
        "min_lat": -10.0,
        "max_lat": 5.0,
        "min_lon": -75.0,
        "max_lon": -45.0
...
class ArchaeologyConfig:
    ...
    elevation_range: Dict[str, float] = field(default_factory=lambda: {
        "min": 50.0,
        "max": 500.0
    })
    
    # Vegetation analysis parameters
    ndvi_thresholds: Dict[str, float] = field(default_factory=lambda: {
        "bare_soil": 0.1,
        "sparse": 0.3,
        "moderate": 0.5,
        "dense": 0.7
    ...
    site_indicators: List[str] = field(default_factory=lambda: [
        "platform", "plaza", "mound", "earthwork", "terrace",
        "canal", "causeway", "circular_feature", "linear_feature"
    ])
...
class ConfigManager:
    ...
    def validate_config(self) -> Dict[str, List[str]]:
        ...
        for path_name, path in [
            ...
            if not path.exists():
                issues["warnings"].append(f"{path_name} directory does not exist: {path}")
        ...
        threshold_values = [thresholds["bare_soil"], thresholds["sparse"], 
                          thresholds["moderate"], thresholds["dense"]]
